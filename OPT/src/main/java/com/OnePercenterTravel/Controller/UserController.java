...
public class UserController {
    ...
    // YOU ONLY AUTOWIRE SERVICES IN CONTROLLER NEVER THE REPO!!!!!!!!!!!!!!!!!!!!
    @Autowired
    UserService userService;

    // 4 main rest calls, Get, Put, Post, Delete
    // Get mappings are when you are going straight to a url
    // no form submission at all

    @GetMapping("/")
    // This function will run whebever someone goes to the / url (http://localhost:8080/)
    ...
    public String signUp(Model model) {
        ...
        // We must set a model attribute on the the page, so our form can have a 
        // blank objest to contruct with the form data
    ...
    @PostMapping("/sign-up")                               // for the model
    ...
    public String signIn(@ModelAttribute("user") User user, HttpSession session) {
        ...
        try {
            ...
            // i store the users email on a cookie for future use to re-verify
